...
function udp_mini() {
  ...
  wget raw.githubusercontent.com/raffasyaa/skt/main/Fls/limit.sh && chmod +x limit.sh && ./limit.sh
  cd
...
function ins_backup() {
  ...
  # Periksa REPO variabel
  if [[ -z "$REPO" ]]; then
      echo "Variabel REPO belum diatur. Silakan tetapkan URL repositori."
      exit 1
  fi

  # Instalasi Dependensi
  apt update && apt install -y rclone git make msmtp-mta ca-certificates bsd-mailx

  # Konfigurasi Rclone
  printf "q\n" | rclone config
  wget -O /root/.config/rclone/rclone.conf "${REPO}Cfg/rclone.conf" || {
      echo "Gagal mengunduh konfigurasi rclone. Periksa URL Anda."
      exit 1
  }

  # Instalasi Wondershaper
  git clone https://github.com/LunaticBackend/wondershaper.git /tmp/wondershaper
  cd /tmp/wondershaper
  sudo make install
  cd
...
port 587
auth on
user skartistore@gmail.com
from skartistore@gmail.com
password Ganesa133@#::
logfile ~/.msmtp.log
EOF
  chmod 600 /etc/msmtprc

  # File kosong di /home/files
  touch /home/files
...
EOF
  ...
  print_success "UDP-CUSTOM VPN Installed"
  clear
}
...
echo -e ""
sleep 2
read -p "Reboot dulu yuk, biar gk error (Enter)."
clear
reboot
